  ...
  return (
    <div>
      ...
      <Container fluid>
        <Row>
          <Col>
            <h1>{postagem ? postagem.titulo : 'Carregando postagem...'}</h1>
            <p>{postagem ? postagem.conteudo : 'Carregando conteúdo...'}</p>
          </Col>
        </Row>
        <Row>
          <Col>
            <Button onClick={() => setIsModalOpen(true)}>comments</Button>
            <Button onClick={() => navigate(`/postagem/${id}/comentarios`)}>Comentarios</Button>
          </Col>
        </Row>
      </Container>
    </div>
